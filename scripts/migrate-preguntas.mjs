// ============================================================================
// Migración: extrae el banco de preguntas embebido en src/App.jsx (evaluación
// calificada HOY en el navegador, con la respuesta correcta visible por
// consola) y genera el SQL de inserción a public.preguntas.
//
// Cada video_id se resuelve en el propio SQL mediante subconsulta
// (sacramentos.slug, videos.orden) — nunca se hardcodean UUIDs aquí.
//
// Uso:
//   node scripts/migrate-preguntas.mjs > scripts/migracion_preguntas.sql
//
// Las constantes de abajo (TC1_MODULES, TC2_CONFESION, TC2_UNCION, COURSES, Q
// y el bloque de reutilización) son una copia literal de src/App.jsx
// (líneas 264-596 al momento de generar este script). No se inventó ningún
// mapeo: si el banco de preguntas cambia en App.jsx, hay que volver a copiar
// esas mismas constantes aquí antes de re-ejecutar.
// ============================================================================

// ─── COPIA LITERAL DE src/App.jsx (líneas 264-366): metadata de videos ────
const TC1_MODULES=[
  {id:"m1",num:"I",
...
// ─── COPIA LITERAL DE src/App.jsx (líneas 368-596): banco de preguntas ────
const Q={
  t1:[
...
// Secciones (secId de negocio == sacramentos.slug real en la BD). Se arman
// directamente con el slug real de la BD, NO con las claves de SEC_META en
// App.jsx (que tienen una inconsistencia conocida: SEC_META usa la clave
// "presacramental" pero COURSES / buildSeq / la BD usan "prebautismal").
const SECTIONS=[
...
const out=[];
out.push("-- ============================================================");
out.push("-- Migración: banco de preguntas de src/App.jsx -> public.preguntas");
out.push("-- Generado por scripts/migrate-preguntas.mjs — NO editar a mano.");
out.push("-- Cada video_id se resuelve por (sacramentos.slug, videos.orden);");
out.push("-- no hay UUIDs hardcodeados.");
out.push("--");
out.push("-- ⚠️ pregunta_en / opcion_*_en: el banco original en App.jsx NO tiene");
out.push("--    traducción al inglés (todo el texto está en español). Se duplica");
out.push("--    el español en las columnas _en como placeholder, para no dejar");
out.push("--    columnas NOT NULL vacías y no romper la vista en inglés. Marcar");
out.push("--    como pendiente de traducción real.");
out.push("-- ============================================================");
out.push("");
out.push("BEGIN;");
out.push("");
...
for(const {slug,videos} of SECTIONS){
  for(const v of videos){
    const qs=Q[v.id];
    if(!qs||!qs.length){ faltantes.push(`${slug}/${v.id} (orden ${v.o})`); continue; }
    out.push(`-- ${slug} · orden ${v.o} · ${v.id} — ${v.es}`);
    ...
    qs.forEach((q,i)=>{
      ...
      out.push(
        ...
        "(video_id, pregunta_es, pregunta_en, opcion_a_es, opcion_a_en, opcion_b_es, opcion_b_en, "+
        "opcion_c_es, opcion_c_en, opcion_d_es, opcion_d_en, respuesta_correcta, puntaje, orden, activo) VALUES ("+
        `${videoSub}, '${esc(q.q)}', '${esc(q.q)}', `+
        `'${esc(q.o.a)}', '${esc(q.o.a)}', '${esc(q.o.b)}', '${esc(q.o.b)}', `+
        `'${esc(q.o.c)}', '${esc(q.o.c)}', '${esc(q.o.d)}', '${esc(q.o.d)}', `+
        `'${q.k}', 2, ${orden}, true);`
      );
      total++;
    });
...
out.push("-- Salvaguarda: si alguna fila no encontró su video_id (mapeo roto),");
out.push("-- abortar la transacción entera en vez de dejar preguntas huérfanas.");
out.push("DO $$");
out.push("DECLARE huerfanas INT;");
out.push("BEGIN");
out.push(`  SELECT COUNT(*) INTO huerfanas FROM public.preguntas WHERE video_id IS NULL;`);
out.push("  IF huerfanas > 0 THEN");
out.push("    RAISE EXCEPTION 'Migración abortada: % preguntas sin video_id resuelto', huerfanas;");
out.push("  END IF;");
out.push("END $$;");
out.push("");
out.push("COMMIT;");
out.push("");
out.push(`-- Total de filas generadas: ${total} (esperado: 54 videos × 5 preguntas = 270)`);
if(faltantes.length){
  out.push(`-- ⚠️ Videos SIN banco de preguntas (revisar Q en App.jsx): ${faltantes.join(", ")}`);
}
