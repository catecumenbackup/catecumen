...
- **`src/components/`** — pantallas/componentes extraídos, cada uno con su `*.test.jsx`: `EstrellasInput`, `AgendaTab`, `MensajesTab`, `EncuadreModal`, `LoginModal`, `ValidarConstanciaModal`.
- **Carga diferida (`React.lazy` + `<Suspense>`):** `AgendaTab`, `MensajesTab`, `EncuadreModal`, `LoginModal`, `ValidarConstanciaModal` — chunks aparte, fuera del bundle inicial. Es el "buen" troceo: pantallas que se abren DESPUÉS, no en la ruta crítica (distinto del vendor splitting que empeoró el arranque).
