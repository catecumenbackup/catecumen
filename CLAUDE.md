...
- **Etiquetas** — etiquetas configurables ("Próximamente", color rojo/dorado editable) sobre las opciones del modal de registro y los sacramentos. Al activar una etiqueta se **muestra el chip**; que además **bloquee** el acceso es una decisión aparte del admin: columna `bloquea` (bool) en `opciones_etiquetas` — si `bloquea=true` (por defecto) la opción/sacramento queda inhabilitado (cliente + validación server-side en `crear-sesion-pago`, que filtra `activo AND bloquea`); si `bloquea=false` solo se muestra la etiqueta sin impedir el clic. El hook `useEtiquetasOpciones` devuelve `bloquea`; los consumidores separan "hay etiqueta" (chip) de "bloqueado" (deshabilita). Panel → Etiquetas: selector "Bloquea acceso" en el editor. **Requiere REDESPLEGAR `crear-sesion-pago`.** Tabla `opciones_etiquetas`; frontend lee `obtener_etiquetas_opciones` (hook `useEtiquetasOpciones` en `src/hooks/useEtiquetasOpciones.js`, usado por `FilterModal`, `SacSelectModal` y `DirectorioButton`). El botón **"Buscar parroquia afiliada"** de la bienvenida es etiquetable/bloqueable con la clave **`buscar_parroquia`** (sembrada en `admin-etiquetas.sql`, nombre en `ETQ_NOMBRES` del panel). **Bienvenida:** "Registrarme" (dorado) e "Iniciar sesión" (azul) comparten fila; "Buscar parroquia" (verde) va aparte; "Omitir presentación" quedó bajo el contador de diapositivas. RPCs: `admin_listar_etiquetas`, `admin_guardar_etiqueta`, `admin_etiqueta_activo`. SQL: `scripts/admin-etiquetas.sql`.
- **Avisos** (pestaña 🔔, badge de no leídos) — notificaciones para el admin: nuevos registros (pago confirmado en `activar-pago`, beca/afiliación en `PaymentModal.handleFree`), **preinscripciones** (edge `preinscribir`), **soporte** (`SoporteModal` al enviar) y **consultas** al catequista (`ConsultarCatequistaModal`, además del mensaje interno). Tabla `notificaciones_admin(tipo,titulo,detalle jsonb,ref_id,leida,creado)` con RLS **solo INSERT** (anon+authenticated; las edge functions insertan con service-role); lectura solo por RPCs `admin_notificaciones_listar`/`admin_notificaciones_no_leidas`/`admin_notificacion_leida`/`admin_notificaciones_leer_todas` (es_admin). SQL en `scripts/notificaciones-admin.sql`. El badge se refresca al entrar y cada 60s. Nota: la notificación de registro pagado se emite solo desde `activar-pago` (no el webhook) para evitar duplicados. **Correo (Resend):** además del aviso en el panel se envía un correo a `admin@catecumen.com` (env `ADMIN_NOTIF_EMAIL`). Los eventos del **cliente** (soporte, consulta, registro gratuito) llaman a la edge function **`avisar-admin`** (inserta el aviso + correo); los del **servidor** (`preinscribir`, `activar-pago`) insertan el aviso y envían su propio correo. Requiere secret `RESEND_API_KEY` (y dominio verificado en Resend) + **REDESPLEGAR `avisar-admin`, `preinscribir`, `activar-pago`**. Si `RESEND_API_KEY` no está, el aviso del panel funciona igual (el correo se omite).
...
- **Preinscripción** (pestaña 📝) — control del "modo preinscripción". Con el modo **activo**, el flujo de alumno (solo alumnos, no organizaciones) termina en "Reserva tu lugar sin costo" en vez de pago: crea la cuenta con `usuarios.estado_inscripcion='preinscrito'` y el usuario ve una **pantalla de espera** (`components/PreinscripcionEspera.jsx`) al entrar. Ajuste global en tabla `ajustes(clave,valor jsonb)` clave `preinscripcion` = `{activa, mensaje:{es..it}}`; el frontend lo lee con la RPC pública `obtener_ajuste`, el admin lo guarda con `admin_guardar_ajuste` (es_admin+admin_log). La pestaña: interruptor ON/OFF, editor del mensaje de espera (6 idiomas), lista de preinscritos (`admin_preinscritos_listar`) con búsqueda/contador/**CSV**, y "Mensaje a todos" (itera `admin_enviar_a_usuario`). SQL en `scripts/preinscripcion.sql` (añade `ajustes`, `estado_inscripcion`, las RPCs). **Fase 2 (conversión al abrir) — IMPLEMENTADA:** al desactivar el modo, cuando un preinscrito inicia sesión (`handleLoginSuccess`): si su registro es **gratuito** (beca 100%, `formacion_gratuita`) se autoactiva con la RPC `activar_mi_preinscripcion` (SECURITY DEFINER, solo activa gratuitos); si es **de pago** va a `PaymentModal` en **modo conversión** (`conversion`+`usuarioId`) usando el precio guardado en `usuarios.importe_previsto`. La conversión reutiliza el flujo de Stripe de forma **aditiva**: `crear-sesion-pago` acepta `conversion`/`usuario_id` (no exige contraseña ni valida correo duplicado, marca el `registros_pendientes.payload`), y `activarRegistro` en `activar-pago` y `stripe-webhook` tiene una rama que, si `payload.conversion`, **ACTUALIZA** la cuenta existente a `estado_inscripcion='activo'` (idempotente por el filtro estado) en vez de crear una nueva. El admin también puede activar manualmente (becas/cortesías) con el botón "Activar" (`admin_activar_preinscrito`). SQL: `scripts/preinscripcion-fase2.sql` (columna `importe_previsto`, `activar_mi_preinscripcion`, `admin_activar_preinscrito`). **Requiere REDESPLEGAR** `crear-sesion-pago`, `activar-pago` y `stripe-webhook`, y correr el SQL. **Probar en Stripe TEST antes de abrir** (es el flujo de pago, máximo riesgo).
- **Afiliados** — alta/baja/suspensión de **parroquias, diócesis y centros de tratamiento de adicciones** afiliados. Lista unificada (tipo/situación/búsqueda) con acciones **Aprobar** (alta, `aprobada=true`), **Suspender** (`suspendida=true`, se oculta del directorio pero se conserva), **Reactivar** y **Borrar** (baja definitiva). Situación derivada: `pendiente` (aprobada=false, suspendida=false) / `activa` (aprobada=true) / `suspendida`. RPCs `admin_afiliados_listar(p_tipo,p_estado,p_q)` (union de `parroquias`+`diocesis`+`centros_adiccion`; `p_tipo` acepta `parroquia`/`diocesis`/`centro`) y `admin_afiliado_estado(p_tipo,p_id,p_accion)` (`SECURITY DEFINER`, `es_admin()` + `admin_log`). SQL en `scripts/admin-afiliados.sql` (añade `suspendida` a las tres tablas y `aprobada`/`created_at` a `centros_adiccion`; los centros usan `calle`+`numero` como dirección y no tienen clero). El directorio público (`buscar_afiliados_*`) solo une parroquias+diócesis y filtra `aprobada=true`; los centros NO aparecen en el buscador público, solo en la gestión de admin. **Afiliación "otro"** (organización/institución no prevista: movimientos, colegios, universidades, capellanías, fundaciones, comunidades religiosas…): opción `otro` en `FilterModal` (afiliación), encuadre `ENCUADRES.otro`, formulario `components/RegisterOtroForm.jsx` (campo clave **`tipoOrg`** = tipo de organización que indica el interesado) → tabla `organizaciones_otro` (RLS: INSERT anon; lectura solo admin). Wiring en App: `handleFilterSelect`/`isOrgFlow`/`handleOrgRegisterNext` (rama `orgType==="otro"`). En admin, `admin_afiliados_listar` la une como `tipo='otro'` con columna extra `tipo_org`; `admin_afiliado_estado` acepta `p_tipo='otro'`. La pestaña muestra el tipo junto al nombre (icono 🏢) y filtro "Otras organizaciones". No aparece en el directorio público.
...
- **`src/components/Dashboard.jsx`** — panel "Mi Cuenta": progreso global/por sección, datos personales editables, constancias y las pestañas Agenda/Mensajes (que carga diferidas con su propio `lazy`+`Suspense`). Depende de `data/course.js`, `SecIcon`, `fields.jsx` (incl. `PhoneField`), `supabaseClient` y `ui/i18n`. Diferida. Probada en `Dashboard.test.jsx`. **Guardar cambios de perfil (ago 2026):** "Guardar cambios" **sí persiste** en la BD — `handleSave` es async, `onUpdate` (en App.jsx) llama a la RPC `actualizar_mi_perfil` (columnas de perfil email/teléfono/parroquia) y a `supabase.auth.updateUser` (contraseña y correo de login; el correo requiere confirmación por email → muestra aviso). Antes solo actualizaba estado local (no guardaba nada). SQL: `scripts/actualizar-perfil.sql` (SECURITY DEFINER, acotada a `auth.uid()` + lista blanca; compatible con `rls-usuarios.sql`, no necesita política de UPDATE del cliente). Muestra estados de guardado/error y deshabilita el botón mientras guarda.
...
- **Login de admin en la app principal → redirige a `/admin/`.** En `handleLoginSuccess`, si no hay perfil en `usuarios` pero la RPC `es_admin()` da true, `window.location.href="/admin/"` (antes rebotaba a la bienvenida). El panel usa `storageKey` propio, así que pedirá login de nuevo (esperado).
- **Gestión de afiliados en el panel** (pestaña ⛪ Afiliados) — ver la sección "Afiliados" arriba. Cubre parroquias, diócesis, centros de adicciones y **"otro"**. SQL: `scripts/admin-afiliados.sql` (idempotente; crea `organizaciones_otro` con RLS+`GRANT INSERT` a anon, añade `suspendida`/`aprobada`/`created_at`, y las RPCs `admin_afiliados_listar`/`admin_afiliado_estado`). **Nota:** una afiliación nueva entra como `pendiente`; hay que **aprobarla** para que salga en el directorio público.
- **Afiliación "otro"** — nueva opción de organización/institución no prevista. Ver sección "Directorio de afiliados".
- **Buscador de afiliados (UX escritorio):** modal anclado a la ventana (`position:fixed`, 24px de gap en los 4 lados, centrado con `width:auto`+`maxWidth`+`margin:auto` — NO fijar `width` con `left/right:0`, se alinea a la izquierda y corta a la derecha); mapa y panel al 50/50; controles comprimidos ("Cerca de mí" en la fila de filtros, país en la fila del buscador por palabra). **Geolocalización más rápida:** `getCurrentPosition` con `maximumAge:300000` (acepta posición reciente en caché; sin esto fuerza un fix nuevo lento) + `enableHighAccuracy:false`, e indicador "Buscando cerca de ti…". **Búsqueda por texto flexible:** `scripts/directorio-flexible.sql` (unaccent + lower + tokens + pg_trgm).
- **`.htaccess` — geolocalización.** El header era `Permissions-Policy: geolocation=()` (bloqueaba la geoloc en silencio, sin prompt). Ahora `geolocation=(self)`. **Debe subirse el `.htaccess` con el `dist/`.**
- **FilterModal "Dinos quién eres":** dos paneles en escritorio (Formación izq / Afiliación der), enlace "Volver al inicio" (prop `onBack`), pop-up de becas **una vez por sesión** (`sessionStorage["catecumen_becas_visto"]`), textos de opciones reescritos.
