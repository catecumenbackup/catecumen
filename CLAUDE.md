...
- **Afiliados** — alta/baja/suspensión de **parroquias y diócesis** afiliadas. Lista unificada (tipo/situación/búsqueda) con acciones **Aprobar** (alta, `aprobada=true`), **Suspender** (`suspendida=true`, se oculta del directorio pero se conserva), **Reactivar** y **Borrar** (baja definitiva). Situación derivada: `pendiente` (aprobada=false, suspendida=false) / `activa` (aprobada=true) / `suspendida`. RPCs `admin_afiliados_listar(p_tipo,p_estado,p_q)` y `admin_afiliado_estado(p_tipo,p_id,p_accion)` (`SECURITY DEFINER`, `es_admin()` + `admin_log`). SQL en `scripts/admin-afiliados.sql` (añade la columna `suspendida`). El directorio público sigue filtrando `aprobada=true`, así que pendientes/suspendidas no aparecen sin tocar sus RPCs.
